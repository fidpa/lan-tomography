...
in a documented format you can re-analyse as often as the fault requires — with a
...
> measurements that look plausible and are wrong — and the **tomography idea**:
> choosing probe locations and a target matrix so the sections you care about
> become individually determinable, instead of collecting targets by gut
> feeling.
...
simply start further along. The target matrix, the pitfalls and the verdict
...
one rack, measured from one probe, tell you about one uplink.

In the campaign this came from, three probes and eight targets gave rank 9
against 11 sections — exactly one section individually determinable. Adding one
target that *terminates on a switch* raised it to 10 and made four determinable.

**Some targets must carry no verdict.** A switch answers ICMP from its
management CPU, not its forwarding path. Measured: apparent outages up to 24
seconds while every target *behind* that switch stayed clean. Keep the target —
it raises the rank — but never let it decide anything.

**"No data" is not "no loss".** Every analysis function here returns `None`, not
`0`, for an unmeasured window. Collapsing the two produces a clean bill of
health for a window nobody measured.

## Requirements

- Linux, Bash 4.0+, Python 3.11+
- `iputils-ping`, `tcpdump`, `ethtool`, `zstd`
...
- Optional: `impacket`, `python-evtx` (`requirements.txt`) — needed **only** by
  `src/contrib/evtx-peek.py`. Everything on the network side runs on the Python
...
| | |
|---|---|
| [Pitfalls](docs/explanation/pitfalls.md) | 44 ways these measurements mislead |
...
- **SNMP counters go blind exactly when it matters.** Measured: missing for 12
  of 18 flood minutes, and not randomly. A quiet timeline is not an all-clear.
...
1.0.0 is reserved for the point where the configuration keys, log formats and
role vocabulary are stable enough that changing them would deserve a major
